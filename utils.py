...
def set_torch_device(gpu_ids):
    """Returns the correct torch device or model parallelism setup.

    Args:
        gpu_ids: [list] list of GPU ids. If empty, cpu is used.
    """
    if len(gpu_ids) == 1:  # 하나의 GPU만 사용하는 경우
        return torch.device(f"cuda:{gpu_ids[0]}")  # 첫 번째 GPU
    elif len(gpu_ids) > 1:  # 여러 GPU를 사용할 경우
        # DataParallel을 사용할 때, 기본적으로 첫 번째 GPU에서 모델을 복제
        device = torch.device(f"cuda:{gpu_ids[0]}")
        return device  # DataParallel을 사용하여 모델을 GPU에 분산
    return torch.device("cpu")  # GPU가 없으면 CPU로
